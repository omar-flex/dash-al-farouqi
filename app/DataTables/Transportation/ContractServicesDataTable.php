...
class ContractServicesDataTable extends DataTable
{
    ...
    public function dataTable()
    {
        ...
        if ($response->ok()) {
            $data = collect(Arr::get($response->json(), 'result.details.transportation_lines'));
        } else {
            $data = collect([]);
        }
    ...
    {
        return $this->builder()
            ->setTableId('transportation_lines-table')
            ->columns($this->getColumns())
            ->minifiedAjax(route('contract.contractServicesDataTable', ['transportation_id' => $this->transportation_id]))
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('name')->name('name')->title('Seasonality'),
