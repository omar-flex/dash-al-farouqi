...
class GuidesDataTable extends DataTable
{
    ...
    public function dataTable()
    {
        ...
        ]))->get('https://ashaxisdmc.odoo.com/api/tariff/get_guide');
        ...
        return (new CollectionDataTable($data))
            ...
            ->editColumn('partner_name', function ($guide) {
                return '<a href="' . route('contract.guides.show', Arr::get($guide, 'id')) . '">' . Arr::get($guide, 'partner_id.name') . '</a>';
            })
            ->addColumn('language_name', function ($guide) {
                $guide = Arr::get($guide, 'language');
                return Arr::get($guide, 'name', '-');
            })
            ->addColumn('action', function ($guide) {
                return view('pages.apps.contract.guides.columns._actions', compact('guide'));
            })
            ->addIndexColumn();
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('language_name')->name('language.name')->title('language'),
