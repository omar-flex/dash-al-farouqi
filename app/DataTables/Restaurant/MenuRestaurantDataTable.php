...
class MenuRestaurantDataTable extends DataTable
{
    ...
    public function dataTable()
    {
        ...
        if ($response->ok()) {
            $data = collect(Arr::get($response->json(), 'result.details.menu_lines'));
        } else {
            $data = collect([]);
        }
    ...
    {
        return $this->builder()
            ->setTableId('menu-restaurant-table')
            ->columns($this->getColumns())
            ->minifiedAjax(route('contract.menuRestaurantDataTable', ['restaurant_id' => $this->restaurant_id]))
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('menu_name')->name('menu_name')->title('Name'),
            Column::make('per_person')->addClass('text-center'),
            Column::make('exters')->addClass('text-center'),
        ];
    }
}
