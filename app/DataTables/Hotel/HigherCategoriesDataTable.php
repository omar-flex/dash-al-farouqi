...
class HigherCategoriesDataTable extends DataTable
{
    ...
    public function dataTable()
    {
        ...
        if ($response->ok()) {
            $data = collect(Arr::get($response->json(), 'result.details.higher_categories'));
        } else {
            $data = collect([]);
        }
    ...
    {
        return $this->builder()
            ->setTableId('higher_categories-table')
            ->columns($this->getColumns())
            ->minifiedAjax(route('contract.higherCategoriesDataTable', ['hotel_id' => $this->hotel_id]))
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('seasonality_lines')->title('Seasonality'),
