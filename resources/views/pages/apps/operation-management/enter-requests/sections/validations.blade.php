            ...
            $('#formValidations').submit(function (e) {
                ...
                if (clickedButton === 'btn-submit') {
                    Swal.fire({
                        ...
                        if (result.value) {
                             if (sumCustomValue() !== {{$enterRequest->total_cost}}){
                                toastr.error('Sum Customs Value (' + sumCustomValue() + ') Must Equal Customs Value Sum ({{$enterRequest->total_cost}})')
                                $("#btn-submit,#btn-draft").prop("disabled", false)
                                return false;
                            }
                            if (sumGrossWeight() !== {{$enterRequest->gross_weight}}){
                                toastr.error('Sum Gross Weight (' + sumGrossWeight() + ') Must Equal Gross Weight Sum ({{$enterRequest->gross_weight}})')
                                $("#btn-submit,#btn-draft").prop("disabled", false)
                                return false;
                            }
                            if (sumNetWeight() !== {{$enterRequest->net_weight}}){
                                toastr.error('Sum Net Weight (' + sumNetWeight() + ') Must Equal Net Weight Sum ({{$enterRequest->net_weight}})')
                                $("#btn-submit,#btn-draft").prop("disabled", false)
                                return false;
                            }
