        ...
        <div class="row px-3" data-repeater-products-item>
            ...
            <div class="col-2 mb-2">
                <input class="form-control form-control-solid-bg form-control-sm codes" disabled type="text"
                       value="{{$item?->warehouseItem?->Product?->name}}" title="{{$item?->warehouseItem?->Product?->name}}"/>
            </div>
            <div class="col-1 mb-2">
                <input class="form-control form-control-solid-bg form-control-sm codes"
                       disabled type="text" value="{{$item?->warehouseItem?->Product?->UnitMeasure?->name}}" title="{{$item?->warehouseItem?->Product?->UnitMeasure?->name}}"/>
            </div>
            <div class="col-1 mb-2">
                <input class="form-control form-control-sm form-control-solid-bg" disabled value="{{$item?->quantity}}" title="{{$item?->quantity}}"/>
            </div>
            <div class="col mb-2">
                <input type="number" step="any" min="0" class="form-control form-control-sm"
                      disabled
                       placeholder="Customs Tariff Code" value="{{$item?->warehouseItem?->custom_tariff_code}}" title="{{$item?->warehouseItem?->custom_tariff_code}}"
